  ...
  try {
      ...
      return NextResponse.json({
        ...
        message:
          rowsToInsert.length > 0
            ? 'Validation successful. New facility asset rows are ready for insertion.'
            : 'Validation successful. No new facility asset rows to insert.'
      });
    }
    ...
    if (rowsToInsert.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: 'No new facility assets to insert. All uploaded asset numbers already exist.',
          data: {
            skippedExisting: skippedExisting.map((row) => ({
              assetnumber: row.assetnumber,
              sourceRow: row.sourceRow
    ...
    return NextResponse.json({
      ...
      message: `Inserted ${result.insertedCount} facility asset(s) successfully.`
    });
  } catch (error: unknown) {
    console.error('Error in facility assets bulk import:', error);
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to process facility assets bulk import.',
        details: message
      },
      { status: 500 }
    );
