...
export async function GET(request: Request) {
  try {
    ...
    console.error('Failed to fetch facility assets:', error);
    return NextResponse.json({ error: 'Failed to fetch facility assets' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (existing) {
      return NextResponse.json(
        { error: `Facility asset with asset number "${doc.assetnumber}" already exists.` },
        { status: 409 }
      );
    }
    ...
    console.error('Failed to create facility asset:', error);
    return NextResponse.json({ error: 'Failed to create facility asset' }, { status: 500 });
  }
}
