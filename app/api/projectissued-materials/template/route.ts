...
export async function GET(request: Request) {
  try {
    ...
    // Define headers for materials - matching the Add Material form
    const headers = [
      'Material Code',
    ...
    const columnWidths = [
      ...
      { wch: 18 }, // Gatepass Number
      { wch: 25 }, // Received By Employee Number
      { wch: 25 }, // Received By Employee Name
      { wch: 30 }  // Remarks
    ];
    ...
    console.error('Failed to generate materials template:', err);
    return NextResponse.json(
