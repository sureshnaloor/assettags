...
function normalizePortableTypeInput(raw: string): string {
...
function validateRows(rows: BulkRow[]) {
  ...
  rows.forEach((row, index) => {
    ...
    const portableTypeRaw = normalizePortableTypeInput(String(row.portableType ?? ''));
    if (portableTypeRaw && !PORTABLE_TYPES.has(portableTypeRaw)) {
      errors.push(
        `Row ${rowNo}: Invalid Portable Type "${row.portableType}". Use pre_engineered, container_20, container_40, prefabricated_sheet, or leave empty.`
      );
      return;
    }
    ...
    normalizedRows.push({
      ...
      portableType: portableTypeRaw,
      installationLocation: normalizeText(row.installationLocation),
      sourceRow: rowNo
    });
  });
  ...
  try {
      ...
      return NextResponse.json({
        ...
        message:
          rowsToInsert.length > 0
            ? 'Validation successful. New portable asset rows are ready for insertion.'
            : 'Validation successful. No new portable asset rows to insert.'
      });
    }
    ...
    if (rowsToInsert.length === 0) {
      return NextResponse.json(
        {
          success: false,
          error: 'No new portable assets to insert. All uploaded asset numbers already exist.',
          data: {
            skippedExisting: skippedExisting.map((row) => ({
              assetnumber: row.assetnumber,
              sourceRow: row.sourceRow
    ...
    return NextResponse.json({
      ...
      message: `Inserted ${result.insertedCount} portable asset(s) successfully.`
    });
  } catch (error: unknown) {
    console.error('Error in portable assets bulk import:', error);
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json(
      {
        success: false,
        error: 'Failed to process portable assets bulk import.',
        details: message
      },
      { status: 500 }
    );
