...
function validateRows(rows: BulkFixedAssetRow[]) {
  const errors: string[] = [];
  const normalizedRows: BulkFixedAssetRow[] = [];
...
export async function POST(request: Request) {
  try {
    ...
    const collection = db.collection('fixedassets');
      ...
      return NextResponse.json({
        ...
        message:
          rowsToInsert.length > 0
            ? 'Validation successful. New fixed asset rows are ready for insertion.'
            : 'Validation successful. No new fixed asset rows to insert.'
      });
    }
    ...
    return NextResponse.json({
      ...
      message: `Inserted ${result.insertedCount} fixed asset(s) successfully.`
    });
  } catch (error: any) {
