  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6 min-h-screen bg-gradient-to-br from-blue-50 to-sky-100 dark:from-slate-900 dark:to-slate-800">
      <div className="flex items-center gap-4">
        <h1 className="flex-1 text-xl font-semibold text-slate-800 dark:text-slate-200">MME Search by Year of Acquisition</h1>
      </div>
      ...
      <div className="mb-6 p-6 bg-white/70 dark:bg-slate-800/70 backdrop-blur-sm rounded-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg">
        ...
        <p className="mt-3 text-xs text-slate-600 dark:text-slate-400">Select a year to search for equipment acquired in that year.</p>
      </div>
        ...
        {loading ? (
          ...
          <>
            <div className="px-4 py-2 border-b border-slate-200/50 dark:border-slate-700/50">
              <p className="text-sm text-slate-600 dark:text-slate-400">Found {data.length} equipment record{data.length !== 1 ? 's' : ''} acquired {selectedYear === 'pre-2010' ? 'before 2010' : `in ${selectedYear}`}</p>
            </div>
