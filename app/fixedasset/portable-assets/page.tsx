  ...
  const columns: ColumnDef<PortableAsset>[] = [
    {
      ...
      cell: ({ row }) => (
        <Link
          href={`/fixedasset/portable-assets/${encodeURIComponent(row.original.assetnumber)}`}
    ...
    },
    {
      ...
      header: () => <span className={backgroundStyles.textColor}>Portable type</span>,
      cell: ({ row }) => (
        <span className={`text-[12px] ${backgroundStyles.cellText}`}>
          {PORTABLE_TYPE_LABELS[row.original.portableType ?? ''] ??
            row.original.portableType ??
            '—'}
        </span>
      )
    },
  ...
  return (
    <div className={backgroundStyles.container}>
      <div className="relative z-20 flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6 min-h-screen">
        <div className="mb-4">
          ...
          >
            ...
            <p className={`${backgroundStyles.headerSubtitle} text-lg`}>
              Add portable assets individually or in bulk. Open an asset for installation location and modifications.
            </p>
          </div>
        </div>
        ...
        >
          <h2 className={`text-lg font-semibold ${backgroundStyles.textColor}`}>Add portable asset</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className={`block text-sm mb-1 ${backgroundStyles.headerSubtitle}`}>Portable type</label>
              <select
                className={inputClass}
                value={form.portableType}
                onChange={(e) =>
                  setForm((f) => ({ ...f, portableType: e.target.value as PortableTypeValue | '' }))
                }
              >
            ...
            <div className="md:col-span-2">
              <label className={`block text-sm mb-1 ${backgroundStyles.headerSubtitle}`}>
              ...
              <input
                ...
                placeholder="Where the unit is installed on site"
              />
            </div>
          </div>
        ...
        <div className={`p-6 ${backgroundStyles.panelBg} rounded-xl shadow-lg`}>
          ...
          <p className={`mt-2 text-sm ${backgroundStyles.headerSubtitle}`}>
            Leave filters short or empty to list all portable assets.
          </p>
        </div>
        ...
        <div className={`rounded-xl ${backgroundStyles.resultsBg} shadow-xl`}>
          <div className={`px-4 py-3 border-b border-white/10 ${backgroundStyles.panelBg}`}>
            <h2 className={`text-lg font-semibold ${backgroundStyles.textColor}`}>Portable asset records</h2>
          </div>
          {loading ? (
            ...
            <div className={`text-center py-10 ${backgroundStyles.emptyText}`}>
              No portable assets yet. Add one above or use bulk insert.
            </div>
          ) : (
        ...
        {showBulkInsertModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`${backgroundStyles.panelBg} w-full max-w-4xl rounded-2xl p-6 shadow-xl max-h-[90vh] overflow-y-auto`}>
              <h3 className={`mb-4 text-2xl font-semibold ${backgroundStyles.textColor}`}>Bulk insert portable assets</h3>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className={`${backgroundStyles.panelBg} w-full max-w-2xl rounded-2xl p-6 shadow-xl max-h-[90vh] overflow-y-auto`}>
              <h3 className={`mb-4 text-xl font-semibold ${backgroundStyles.textColor}`}>
                Edit portable asset {editAssetNumber}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setEditForm((f) => ({ ...f, portableType: e.target.value as PortableTypeValue | '' }))
                    }
                  >
                    <option value="">Not set</option>
                    <option value="pre_engineered">Pre engineered</option>
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setEditForm((f) => ({ ...f, installationLocation: e.target.value }))}
                  />
                </div>
              </div>
