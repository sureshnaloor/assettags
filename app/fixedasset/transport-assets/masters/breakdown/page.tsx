...
export default function TransportBreakdownMasterPage() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await fetch('/api/transportassets/maintenance-types/breakdown');
  ...
  const del = async (id: string) => {
    ...
    try {
      setError(null);
      const res = await fetch(`/api/transportassets/maintenance-types/breakdown?id=${encodeURIComponent(id)}`, {
        method: 'DELETE'
      });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a2332] via-[#2d3748] to-[#1a2332] p-6 text-white">
      <div className="mx-auto max-w-3xl space-y-6">
        <div>
          <h1 className="text-2xl font-bold">Breakdown maintenance types</h1>
          <p className="mt-1 text-sm text-white/70">
            Master list for repairs and damage (engine, body, etc.). Used when logging breakdown maintenance on
            transport assets.
          </p>
        </div>
