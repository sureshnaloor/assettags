...
export default function MMESearchByCategoryPage() {
  ...
  useEffect(() => {
    if (selectedCategory) {
      searchEquipment(selectedCategory);
    } else {
      setData([]);
    }
  }, [selectedCategory]);
  ...
  const columns: ColumnDef<Equipment>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <div className="text-[12px] text-white">{row.getValue('assetcategory')}</div>,
    },
    {
      accessorKey: 'assetsubcategory',
      header: 'Subcategory',
      cell: ({ row }) => <div className="text-[12px] text-white">{row.getValue('assetsubcategory') || 'N/A'}</div>,
    },
      ...
      <div className="relative z-20 flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6 min-h-screen">
        ...
        <div className="mb-8">
          <div className="bg-white/10 backdrop-blur-lg border border-white/20 rounded-3xl p-8 hover:bg-white/15 transition-all duration-300">
            <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-white to-teal-400 bg-clip-text text-transparent">
              MME Search by Category
            </h1>
            <p className="text-white/80 text-lg">Search MME equipment by category</p>
          </div>
        </div>
        ...
        <div className="mb-6 p-6 bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl shadow-lg">
          <div className="flex gap-4">
            ...
            >
              <option value="" className="bg-[#1a2332]">Select a category...</option>
          ...
          <p className="mt-3 text-xs text-white/70">Select a category to search for equipment.</p>
        </div>
        ...
        <div className="rounded-xl border border-white/20 bg-white/10 backdrop-blur-lg shadow-xl">
          {loading ? (
            ...
            <div className="text-center py-8 text-white/70">
              {selectedCategory ? 'No equipment found for the selected category' : 'Select a category to search for equipment'}
            </div>
          ) : (
            <>
              <div className="px-4 py-2 border-b border-white/20">
                <p className="text-sm text-white/80">Found {data.length} equipment record{data.length !== 1 ? 's' : ''} for category "{selectedCategory}"</p>
              </div>
