...
export default function MMESubcategoryPage() {
  ...
  const fetchSubcategories = async () => {
    try {
      ...
      const response = await fetch(`/api/subcategories/mme${query}`);
  ...
  const handleDeleteSubcategory = async (id: string) => {
    ...
    try {
      setError(null);
      const response = await fetch(`/api/subcategories/mme?id=${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="min-h-screen bg-slate-900 p-6 text-white">
      <div className="mx-auto max-w-5xl space-y-6">
        <h1 className="text-3xl font-bold">MME Subcategory Management</h1>
